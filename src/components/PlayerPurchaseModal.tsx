    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.message.includes('would revert on-chain') || error.message.includes('Gas estimation failed')) {
          errorMessage = error.message; // Already a user-friendly message from pre-flight check
          ...
          {transactionStatus !== 'pending' && (!showAlert || notificationDismissed) && (
            <div className="p-3 md:p-4 rounded-lg bg-accent/20 space-y-2 md:space-y-3">
              ...
              {tradingPhase === TradingPhase.BondingCurve && launchInfo ? (
                ...
                <div className="flex justify-between gap-1 text-[10px] md:text-xs text-muted-foreground">
                  <span className="flex-shrink-0">Pool Reserves</span>
                  <span className={`text-right min-w-0 truncate ${isMobile ? 'max-w-[60%]' : ''}`}>
                    {(() => {
